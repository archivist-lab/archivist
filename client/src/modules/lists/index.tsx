...
function PreviewPanel({ loading, error, matchCount, sample, warning, accent }: { loading: boolean; error: string; matchCount: number | null; sample: PreviewMember[]; warning: string | null; accent: string }) {
  return <aside className="rounded-2xl border border-white/8 bg-noir-900/60 p-5 xl:sticky xl:top-6 xl:self-start"><div className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/25">Live preview</div><div className="mt-3 flex items-baseline gap-3"><span className="font-display text-5xl" style={{ color: accent }}>{loading ? '…' : matchCount ?? '—'}</span><span className="font-mono text-[9px] uppercase text-white/30">matches</span></div>{error && <p className="mt-3 rounded-lg bg-red-500/10 p-3 text-xs leading-relaxed text-red-300">{error}</p>}{warning && <p className="mt-3 rounded-lg bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-200">{warning}</p>}<div className="mt-5 grid grid-cols-4 gap-2">{sample.slice(0, 12).map(item => <div key={item.tmdbId} title={`${item.title}${item.year ? ` (${item.year})` : ''}`} className="aspect-[2/3] overflow-hidden rounded-lg bg-white/5">{item.posterPath ? <img src={tmdbImage(item.posterPath)} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center px-1 text-center text-[8px] text-white/25">{item.title}</div>}</div>)}</div><p className="mt-4 text-[10px] leading-relaxed text-white/25">Preview is read-only. Saving a list still requires you to approve each new candidate.</p></aside>
}
...
function ItemCard({ item, selected, onSelect, onAction, busy, accent }: { item: ListItem; selected: boolean; onSelect: () => void; onAction: (action: 'add' | 'dismiss') => void; busy: boolean; accent: string }) {
  const actionable = item.status === 'new' || item.status === 'failed' || item.status === 'departed'
  return <div className="overflow-hidden rounded-xl border border-white/8 bg-noir-900/55"><div className="relative aspect-[2/3] bg-white/5">{item.poster_path ? <img src={tmdbImage(item.poster_path)} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center p-4 text-center text-xs text-white/25">{item.title}</div>}{actionable && <label className="absolute left-2 top-2 grid h-7 w-7 place-items-center rounded-lg bg-noir-950/85"><input type="checkbox" checked={selected} onChange={onSelect} className="accent-[#00D4FF]" /></label>}<span className="absolute bottom-2 left-2 rounded bg-noir-950/90 px-2 py-1 font-mono text-[8px] uppercase text-white/65">{item.status.replace('_', ' ')}</span></div><div className="p-3"><div className="truncate text-sm font-semibold text-white/85">{item.title}</div><div className="mt-1 font-mono text-[9px] text-white/30">{item.year ?? '—'} · TMDB {item.tmdb_id}</div>{item.status_reason && <p className="mt-2 line-clamp-2 text-[10px] text-red-300/65">{item.status_reason}</p>}{actionable && <div className="mt-3 grid grid-cols-2 gap-2"><button disabled={busy} onClick={() => onAction('dismiss')} className="rounded-lg border border-white/8 py-2 text-[9px] font-bold uppercase text-white/35 hover:text-red-300">Dismiss</button><button disabled={busy} onClick={() => onAction('add')} className="rounded-lg border py-2 text-[9px] font-bold uppercase" style={{ color: accent, borderColor: `${accent}44`, backgroundColor: `${accent}10` }}>Add</button></div>}</div></div>
}
...
function ListDetail() {
  ...
  const [list, setList] = useState<ArchivistList | null>(null); const [items, setItems] = useState<ListItem[]>([]); const [runs, setRuns] = useState<ListRun[]>([]); const [status, setStatus] = useState<ListStatus>('new'); const [selected, setSelected] = useState<number[]>([]); const [busy, setBusy] = useState(false)

...
| Acquisition and release automation | implemented | **Confirmed —** RSS/recent-release processing, targeted missing searches, durable background item searches with 15-minute result restoration, release parsing/decisions, indexers, download clients, and import jobs exist. Film and episode imports organize downloaded media from stored library metadata rather than making availability-sensitive TMDB calls. |

...
**Depends on:** Cardigann definition loader, Trawl, CoreEvent spine, `@mediastack/core`
...
|---|---|---|
| 0.1 | **Trawl** is Archivist's challenge-solving headless-browser proxy (the FlareSolverr equivalent) — expensive, concurrency-limited, and returns rendered HTML plus cookies. | If Trawl is instead a crawler/scraper, §6 and §8 change substantially. |
...
Archivist should treat indexer reachability as **continuously measured state**, not a config value. Every indexer has N candidate endpoints; the system knows which of them work, which require Trawl, how fast they are, and how reliable they've been — and it picks the best one automatically, failing over mid-search rather than mid-cron-interval.
...
- G2 — Prefer direct HTTP; use Trawl only where the site genuinely requires it, and automatically stop using it when it no longer does.
...
| **Probe** | A single measurement of one endpoint, direct or via Trawl. |
| **Tier** | Coarse quality class of an endpoint: A (works direct), B (works via Trawl), C (degraded), D (dead). |
...
2. The indexer list shows a health pill per indexer: **Direct** (green), **Via Trawl** (amber), **Degraded** (orange), **Down** (red).
...
CREATE TABLE IF NOT EXISTS indexer_endpoint (
  ...
  requires_trawl     INTEGER NOT NULL DEFAULT 0,
...
export interface ProbeOptions {
  allowTrawl: boolean;
  timeoutMs: number;          // default 15_000 direct, 45_000 trawl
  mode: 'browse' | 'search';
...
| Failure class | Detection | Retry via Trawl? | Endpoint verdict | Notes |
|---|---|---|---|---|
...
| `challenge` | 403/503 + CF markers (`cf-mitigated` header, `__cf_chl`, DDoS-Guard body signature) | **Yes** | B if Trawl succeeds, else D | The only class that justifies Trawl |
...
| `parse` | 200, page fetched, row selectors do not match | No | C, **and flag the definition** | Trawl cannot fix a broken selector |
| `empty` | 200, selectors match, zero rows on a browse probe | No | C — degraded | Shadow-ban or empty mirror |

The two rows people get wrong: `rate_limited` must not count against the endpoint (you'll demote your *best* indexer for being used most), and `parse` must not trigger Trawl (you'll burn a browser instance to reproduce the same selector miss, on every endpoint, every sweep).

### 6.4 Probe ladder

```
probeEndpointResolved(endpoint, definition, allowTrawl):

  1. direct = probeEndpoint(endpoint, definition, { allowTrawl: false, mode })

  2. if direct.outcome == 'ok':
       return { tier: 'A', requiresTrawl: false, ...direct }
  ...
  4. if direct.failureClass == 'challenge' and allowTrawl:
       trawl = probeEndpoint(endpoint, definition, { allowTrawl: true, mode })
       if trawl.outcome == 'ok':
         return { tier: 'B', requiresTrawl: true, ...trawl }
       return { tier: 'D', ...trawl }
...
- Global semaphore, default `maxConcurrentTrawlProbes = 2`, separate from the runtime Trawl pool so probing never starves live searches.
- Probe traffic runs at lower priority than search traffic in the Trawl queue.
- Trawl probes are skipped entirely if Trawl is unconfigured or unhealthy; endpoints that previously resolved tier B retain their last-known state and are marked `stale` rather than demoted.

### 6.7 Trawl demotion test

An endpoint at tier B must periodically re-test **direct**, because sites drop bot walls as often as they add them. Every 4th scheduled probe of a tier-B endpoint runs step 1 of the ladder only. Success promotes it to tier A and clears `requires_trawl`.
...
| Tier B (Trawl) | 6h | Every 4th probe is a demotion test (§6.7) |
...
|---|---|---|
| `indexer.endpoint.probed` | endpointId, outcome, tier, latency, viaTrawl | health UI, metrics |
...
- `DIRECT` — accent green
- `TRAWL` — amber, with a small browser glyph
...
| ○ | `https://site.me` | B · Trawl | 2.1s | ▅▁▃▂▅▃ 61% | 3h ago | ⋯ |
...
- Failure classes render as human sentences, not enum values: *"Cloudflare challenge — solved via Trawl"*, *"Page loaded but no results matched the definition — the definition may be out of date."*
...
4. **Trawl exhaustion.** Probe traffic starves live searches of browser instances. Mitigated by a separate semaphore and lower queue priority (§6.6).
...
**Ladder** — assert Trawl is invoked for `challenge` and for **no other class**. This is the assertion that stops a future refactor from quietly making every failure expensive.
...
Trawl demotion testing, definition drift detection, auto-disable, health API.
...
2. **17.2** Should a tier-B (Trawl-required) endpoint ever beat a tier-A endpoint on latency alone? Current scoring says no — the 600-point tier gap is unbridgeable. That's deliberate but worth confirming.

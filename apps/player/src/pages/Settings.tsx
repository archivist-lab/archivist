...
function LivingRoomSettings({ sdk }: { sdk: ArchivistSdk }) {
  ...
  useEffect(() => { requestAnimationFrame(() => document.querySelector<HTMLElement>('[data-focus-id="settings-section-Playback"]')?.focus()) }, [])
  ...
  return <div data-route-scroll className="h-full overflow-y-auto no-scrollbar pb-20">
    <div className="mx-auto flex max-w-[1500px] items-start gap-8">
      ...
      <section className="player-panel min-w-0 max-w-4xl flex-1 p-[clamp(1.5rem,3vw,2.5rem)]">
...
function ProfileSettings({ sdk, activeId }: { sdk: ArchivistSdk; activeId: string }) {
  ...
  return <div className="space-y-5"><p className="text-[12.5px] leading-relaxed text-white/48">Each profile has separate Home hubs, browsing defaults, watched progress and bookmarks.</p><div className="space-y-2">{profiles.map(profile => <div key={profile.id} className={`flex items-center rounded-2xl border p-4 ${profile.id === activeId ? 'player-accent-border player-accent-soft' : 'border-white/10 bg-white/5'}`}><div><strong>{profile.name}</strong><p className="mt-1 text-xs text-white/35">{profile.id}</p></div><div className="ml-auto flex gap-2">{profile.id !== activeId && <button onClick={() => activate(profile.id)} className="player-focusable player-button-primary">Switch</button>}{profile.id !== 'default' && profile.id !== activeId && <button onClick={() => void remove(profile.id)} className="player-focusable player-button-danger">Delete</button>}</div></div>)}</div><div className="flex gap-3 border-t border-white/10 pt-5"><input aria-label="New profile name" value={name} onChange={event => setName(event.target.value)} maxLength={32} placeholder="New profile name" className="player-focusable player-field flex-1" /><button onClick={() => void create()} className="player-focusable player-button-primary">Create profile</button></div>{message && <p role="status" className="text-sm text-pink">{message}</p>}</div>
}
...
function SettingButton({ id, label, active, onPress }: { id: string; label: string; active?: boolean; onPress: () => void }) {
  ...
  return <button {...focusable} className={`player-focusable w-full rounded-xl border px-4 py-3 text-left font-mono text-[10px] font-medium uppercase tracking-[.1em] transition ${active ? 'border-cyan/30 bg-cyan/10 text-cyan' : 'border-transparent text-white/45 hover:border-white/[.06] hover:bg-white/[.035] hover:text-white/70'}`}>{label}</button>
}
...
function HubEditor({ draft, update }: { draft: PlayerPreferencesV1; update: (next: PlayerPreferencesV1) => void }) {
  ...
  return <div className="grid grid-cols-[13rem_minmax(0,1fr)] gap-6">
    <aside className="space-y-2">
      ...
      {hubs.map(hub => <button key={hub.id} onClick={() => setSelectedId(hub.id)} className={`player-focusable flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left ${hub.id === selected.id ? 'player-segment-active' : 'bg-white/5 text-white/65'}`}>
    ...
    <div className="min-w-0 space-y-6">
      <section className="player-panel-inset p-5">
        <div className="grid grid-cols-[5rem_minmax(0,1fr)] gap-3">
          <label className="player-control-label">Icon<input aria-label="Hub icon" value={selected.icon} maxLength={8} onChange={event => patchHub({ icon: event.target.value })} className="player-focusable player-field mt-2 w-full px-3 py-2 text-center text-xl" /></label>
          <label className="player-control-label">Name<input aria-label="Hub name" value={selected.name} maxLength={32} onChange={event => patchHub({ name: event.target.value })} className="player-focusable player-field mt-2 w-full px-3 py-2 text-white" /></label>
        </div>
        <div className="mt-5 flex flex-wrap gap-2">{(['standard','combined','wall'] as PlayerHubLayout[]).map(layout => <button key={layout} onClick={() => patchHub({ layout })} className={`player-focusable player-segment ${selected.layout === layout ? 'player-segment-active' : 'border-white/[.07] bg-white/[.045] text-white/55'}`}>{layout}</button>)}</div>
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <button onClick={() => patchHub({ showSpotlight: !selected.showSpotlight })} className="player-focusable rounded-xl bg-white/8 px-4 py-2">Spotlight: {selected.showSpotlight ? 'On' : 'Off'}</button>
          {selected.showSpotlight && <label className="player-control-label">Source <select value={selected.spotlightWidgetId ?? ''} onChange={event => patchHub({ spotlightWidgetId: event.target.value || null })} className="player-focusable ml-2 player-field px-3 py-2"><option value="">First widget with results</option>{selected.widgets.filter(widget => widget.enabled).map(widget => <option key={widget.id} value={widget.id}>{widget.title}</option>)}</select></label>}
          {selected.id !== 'home' && <button onClick={() => patchHub({ enabled: !selected.enabled })} className="player-focusable rounded-xl bg-white/8 px-4 py-2">Navigation: {selected.enabled ? 'On' : 'Off'}</button>}
        </div>
      ...
      <div className="flex items-center justify-between"><div><h3 className="player-secondary-title text-[1.35rem]">Widgets</h3><p className="mt-1 text-xs text-white/40">Each widget is a server-backed content source with its own presentation and behavior.</p></div><button onClick={addWidget} disabled={selected.widgets.length >= 12} className="player-focusable rounded-xl bg-white px-5 py-2 font-bold text-black disabled:opacity-30">+ Add widget</button></div>
      <div className="space-y-3">{selected.widgets.map((entry, index) => <section key={entry.id} className={`player-panel-inset p-4 ${entry.enabled ? '' : 'opacity-55'}`}>
        <div className="grid grid-cols-2 gap-3">
          <label className="player-control-label">Title<input aria-label={`Widget ${index + 1} title`} value={entry.title} maxLength={48} onChange={event => patchWidget(index, { title: event.target.value })} className="player-focusable player-field mt-2 w-full px-3 py-2 text-white" /></label>
          <label className="player-control-label">Content source<select aria-label={`Widget ${index + 1} content source`} value={entry.source} onChange={event => { const source = event.target.value as PlayerWidgetSource; patchWidget(index, { source, savedFilterId: source === 'saved-filter' ? draft.browsing.savedFilters[0]?.id ?? null : null, downloadMediaTypes: source === 'downloading' ? ['films', 'series'] : [], sortOrder: entry.sort === 'source' ? defaultOrder(source) : entry.sortOrder }) }} className="player-focusable player-field mt-2 w-full px-3 py-2">{SOURCE_GROUPS.map(group => <optgroup key={group.label} label={group.label}>{group.options.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</optgroup>)}{draft.browsing.savedFilters.length > 0 && <optgroup label="Saved views"><option value="saved-filter">Saved filtered view</option></optgroup>}</select></label>
        </div>
        {entry.source === 'saved-filter' && <label className="player-control-label mt-3">Saved view<select aria-label={`Widget ${index + 1} saved view`} value={entry.savedFilterId ?? ''} onChange={event => patchWidget(index, { savedFilterId: event.target.value || null })} className="player-focusable player-field mt-2 w-full px-3 py-2">{draft.browsing.savedFilters.map(saved => <option key={saved.id} value={saved.id}>{saved.name}</option>)}</select></label>}
        {entry.source === 'downloading' && <div className="mt-3"><p className="player-control-label mb-2">Download types</p><div className="flex gap-2">{(['films','series','other'] as const).map(type => { const active = entry.downloadMediaTypes.includes(type); return <button key={type} onClick={() => { const next = active ? entry.downloadMediaTypes.filter(value => value !== type) : [...entry.downloadMediaTypes, type]; if (next.length) patchWidget(index, { downloadMediaTypes: next }) }} className={`player-focusable player-segment ${active ? 'player-segment-active' : 'bg-white/8 text-white/55'}`}>{type}</button> })}</div></div>}
...
function EditorSelect({ label, value, values, onChange, optionLabel = value => value }: { label: string; value: string; values: string[]; onChange: (value: string) => void; optionLabel?: (value: string) => string }) {
  return <label className="player-control-label">{label}<select aria-label={label} value={value} onChange={event => onChange(event.target.value)} className="player-focusable player-field mt-2 w-full px-3 py-2">{values.map(option => <option key={option} value={option}>{optionLabel(option)}</option>)}</select></label>
}
...
function LibrarySettings({ draft, update }: { draft: PlayerPreferencesV1; update: (next: PlayerPreferencesV1) => void }) {
  ...
  return <div className="space-y-5">
    ...
    {contentTypes.map(kind => <div key={kind} className="flex items-center gap-4 player-panel-inset p-5 capitalize"><strong className="w-28">{kind}</strong><label className="flex items-center gap-2 text-sm text-white/55">View<select aria-label={`${kind} view`} value={draft.browsing.defaultViews[kind]} onChange={event => { const view = event.target.value as PlayerView; const next = { ...draft, browsing: { ...draft.browsing, defaultViews: { ...draft.browsing.defaultViews, [kind]: view } } }; if (kind === 'films' || kind === 'series') next.libraries = { ...next.libraries, [kind]: { ...next.libraries[kind], view } }; update(next) }} className="player-field px-3 py-2">{['poster','landscape','wall','list'].map(view => <option key={view}>{view}</option>)}</select></label>{(kind === 'films' || kind === 'series') && <><label className="flex items-center gap-2 text-sm text-white/55">Sort<select aria-label={`${kind} sort`} value={draft.libraries[kind].sort} onChange={event => update({ ...draft, libraries: { ...draft.libraries, [kind]: { ...draft.libraries[kind], sort: event.target.value as PlayerPreferencesV1['libraries'][typeof kind]['sort'] } } })} className="player-field px-3 py-2">{['title','added','year','rating'].map(value => <option key={value}>{value}</option>)}</select></label><label className="flex items-center gap-2 text-sm text-white/55">Order<select aria-label={`${kind} order`} value={draft.libraries[kind].sortOrder} onChange={event => update({ ...draft, libraries: { ...draft.libraries, [kind]: { ...draft.libraries[kind], sortOrder: event.target.value as PlayerSortOrder } } })} className="player-field px-3 py-2"><option value="asc">asc</option><option value="desc">desc</option></select></label><button onClick={() => update({ ...draft, libraries: { ...draft.libraries, [kind]: { ...draft.libraries[kind], hideUnavailable: !draft.libraries[kind].hideUnavailable } } })} className="player-focusable rounded-xl bg-white/8 px-4 py-2">Hide unavailable: {draft.libraries[kind].hideUnavailable ? 'On' : 'Off'}</button></>}</div>)}
    <section className="player-panel-inset p-5"><h3 className="font-semibold">Saved views</h3>{draft.browsing.savedFilters.length === 0 ? <p className="mt-2 text-sm text-white/40">Create saved views from the Options drawer in Films, Series, Episodes or Collections.</p> : <div className="mt-3 space-y-2">{draft.browsing.savedFilters.map(saved => <div key={saved.id} className="flex items-center gap-3 rounded-xl bg-black/20 px-4 py-3"><span className="flex-1">{saved.name}</span><span className="text-xs uppercase text-white/35">{saved.mediaType}</span><button onClick={() => update(removeSavedFilterPreference(draft, saved.id))} className="player-focusable rounded-lg px-3 py-2 text-pink">Delete</button></div>)}</div>}</section>
  </div>
}
...
function PlaybackSettings({ draft, update }: { draft: PlayerPreferencesV1; update: (next: PlayerPreferencesV1) => void }) {
  ...
  return <div className="space-y-6">
    <div><button onClick={() => update({ ...draft, playback: { ...draft.playback, normalizeVolume: !draft.playback.normalizeVolume } })} className="player-focusable player-segment">Normalize loudness: {draft.playback.normalizeVolume ? 'On' : 'Off'}</button><div className="mt-3 flex gap-2">{([-14,-16,-18,-23] as const).map(value => <button key={value} onClick={() => update({ ...draft, playback: { ...draft.playback, targetLufs: value } })} className={`player-focusable player-segment ${draft.playback.targetLufs === value ? 'player-segment-active' : 'border-white/[.07] bg-white/[.045] text-white/55'}`}>{value} LUFS</button>)}</div></div>
    <div className="grid grid-cols-2 gap-4"><label className="player-control-label">Preferred audio language<input aria-label="Preferred audio language" value={draft.playback.preferredAudioLanguage ?? ''} maxLength={16} placeholder="en" onChange={event => update({ ...draft, playback: { ...draft.playback, preferredAudioLanguage: language(event.target.value) } })} className="player-field mt-2 w-full" /></label><label className="player-control-label">Preferred subtitle language<input aria-label="Preferred subtitle language" value={draft.playback.preferredSubtitleLanguage ?? ''} maxLength={16} placeholder="en" onChange={event => update({ ...draft, playback: { ...draft.playback, preferredSubtitleLanguage: language(event.target.value) } })} className="player-field mt-2 w-full" /></label></div>
    <div><p className="player-control-label mb-3">Subtitles on startup</p><div className="flex gap-2">{(['off','forced','preferred'] as const).map(value => <button key={value} onClick={() => update({ ...draft, playback: { ...draft.playback, subtitles: value } })} className={`player-focusable player-segment ${draft.playback.subtitles === value ? 'player-segment-active' : 'border-white/[.07] bg-white/[.045] text-white/55'}`}>{value}</button>)}</div></div>
    <div><p className="player-control-label mb-3">OSD timeout</p><div className="flex gap-2">{([0,3,5,8,10] as const).map(value => <button key={value} onClick={() => update({ ...draft, playback: { ...draft.playback, osdTimeoutSeconds: value } })} className={`player-focusable player-segment ${draft.playback.osdTimeoutSeconds === value ? 'player-segment-active' : 'border-white/[.07] bg-white/[.045] text-white/55'}`}>{value === 0 ? 'Never' : `${value}s`}</button>)}</div></div>

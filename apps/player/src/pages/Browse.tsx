...
export function BrowsePage({ sdk, requestedType }: { sdk: ArchivistSdk; requestedType: PlayerFilterableContentType | 'saved' }) {
  ...
  return <div data-route-scroll className="h-full overflow-y-auto no-scrollbar pb-20">
    <header className="mb-8 flex items-start gap-3"><div><h1 className={`archivist-page-title ${mediaType === 'series' || mediaType === 'episodes' ? 'text-violet' : 'text-cyan'}`}>{title}</h1><p className="archivist-section-label mt-2 text-white/30">{mediaType}</p></div><div className="ml-auto flex items-center gap-2"><button onClick={() => setOptionsOpen(true)} className="player-focusable player-button">Filter and sort</button></div></header>
    <nav aria-label="Jump to title" className="mb-6 flex flex-wrap gap-1">{['#',...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'].map(letter => <button key={letter} aria-pressed={filters.alphabet === letter} onClick={() => { setActiveSavedId(null); setFilters(current => ({ ...current, alphabet: current.alphabet === letter ? null : letter })) }} className={`player-focusable player-segment h-8 min-h-8 min-w-8 px-2 ${filters.alphabet === letter ? 'player-segment-active' : ''}`}>{letter}</button>)}</nav>
    {error && <div role="alert" className="rounded-xl border border-pink/20 bg-pink/10 p-5 font-mono text-[10px] text-pink">{error}</div>}
    ...
    {page && page.items.length === 0 && <div className="player-empty-state mt-20">No items match these filters.</div>}
    {loadingMore && <p className="archivist-section-label mt-4 text-center text-white/35">Loading more…</p>}

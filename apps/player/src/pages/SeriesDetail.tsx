...
export function SeriesDetailPage({ sdk, v2 = false }: { sdk: ArchivistSdk; v2?: boolean }) {
  ...
  const meta = <><span>{series.year ?? 'Year unknown'}</span>{series.certification && <span>{series.certification}</span>}{series.network && <span>{series.network}</span>}<span>{series.availableEpisodeCount}/{series.episodeCount} available</span>{series.seriesStatus && <span>{series.seriesStatus}</span>}</>

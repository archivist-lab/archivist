...
export async function startBackgroundServices(): Promise<() => Promise<void>> {
  ...
  const { registerIndexerReconcileJobs, startIndexerStoreReconciler, stopIndexerStoreReconciler } =
    await import('./services/indexer-bridge.js')

...
import { cancelJob, enqueueJob, type JobRecord } from '../system/event-store.js'
import { registerJobHandler } from '../system/job-runner.js'
import { sendToDownloadClient } from './download-manager.js'
import { getEnabledIndexerInstances, searchViaIndexers, type BridgeSearchResult } from './indexer-bridge.js'
import {
  absoluteQuality,
...
function assertSubject(input: {
  ...
  if (input.mediaType === 'films' && input.subjectType === 'film') {
    ...
    found = db.prepare('SELECT se.id FROM seasons se JOIN series s ON s.id = se.series_id WHERE se.id = ? AND s.library_id = ?').get(input.subjectId, input.libraryId)
  } else if (input.mediaType === 'series' && input.subjectType === 'episode') {
    found = db.prepare('SELECT e.id FROM episodes e JOIN series s ON s.id = e.series_id WHERE e.id = ? AND s.library_id = ?').get(input.subjectId, input.libraryId)
  }
...
function validateFilmRelease(
  ...
  const words = title.replace(/-/g, ' ').replace(/\s+/g, ' ').trim().split(/[\s.]+/).filter(Boolean)
...
function filmQueryPlan(film: any, mode: ItemSearchMode, options: ItemSearchOptions): string[] {
  ...
  const selected = options.tier && options.tier !== 'Any'
    ? [{ name: options.tier, terms: options.tier.includes('1') ? tiers.tier1 : options.tier.includes('2') ? tiers.tier2 : options.tier.includes('3') ? tiers.tier3 : [] }]
    : [
...
async function searchFilm(row: ItemSearchRow, signal: AbortSignal): Promise<{ grabbed: boolean; message: string }> {
  ...
  for (const query of plan) {
    ...
    const raw = await searchViaIndexers(indexers, query, {
      ...
      imdbId: film.imdb_id, tmdbId: film.tmdb_id,
    })
  ...
  if (!best) return { grabbed: false, message: baseline ? 'No eligible upgrade within the quality envelope found' : 'No release within the quality envelope found' }
  ...
  getDb().prepare(`UPDATE films SET status = 'acquiring', info_hash = ?, acquired_at = datetime('now'), updated_at = datetime('now') WHERE id = ? AND library_id = ?`)
...
function validateSeriesRelease(releaseTitle: string, seriesTitle: string): boolean {
  ...
  return normalizeTitle(seriesTitle).split(' ').filter(Boolean).every(word => words.has(word))
}
...
function seriesContext(row: ItemSearchRow): { series: any; seasonNumber?: number; episode?: any } {
  ...
  if (row.subject_type === 'season') {
    const value = db.prepare(`SELECT s.*, se.season_number FROM seasons se JOIN series s ON s.id = se.series_id WHERE se.id = ? AND s.library_id = ?`)
  ...
  const value = db.prepare(`SELECT s.*, e.id AS episode_id, e.season_number, e.episode_number, e.status AS episode_status,
...
function seriesBaseline(context: { series: any; seasonNumber?: number; episode?: any }): CandidateQuality | null {
  ...
  const episodes = (context.seasonNumber != null
    ? getDb().prepare('SELECT * FROM episodes WHERE series_id = ? AND season_number = ?').all(context.series.id, context.seasonNumber)
    : getDb().prepare('SELECT * FROM episodes WHERE series_id = ? AND season_number > 0').all(context.series.id)) as any[]
...
function seriesBase(context: { series: any; seasonNumber?: number; episode?: any }): string {
  if (context.episode) return `${context.series.title} S${String(context.seasonNumber).padStart(2, '0')}E${String(context.episode.episode_number).padStart(2, '0')}`
...
function seriesQueryPlan(context: { series: any; seasonNumber?: number; episode?: any }, mode: ItemSearchMode): string[] {
  ...
  if (context.seasonNumber == null && !context.episode) {
    const seasons = (getDb().prepare('SELECT DISTINCT season_number AS season FROM seasons WHERE series_id = ? AND season_number > 0 ORDER BY season_number')
      .all(context.series.id) as Array<{ season: number }>).map(row => row.season)
...
function releaseMatchesSeriesScope(release: BridgeSearchResult, context: { series: any; seasonNumber?: number; episode?: any }, auto: boolean): boolean {
  ...
  if (!auto) return true
  const airedSeasons = (getDb().prepare(`SELECT DISTINCT season_number AS season FROM episodes WHERE series_id = ? AND season_number > 0
    AND (air_date IS NULL OR substr(air_date, 1, 10) <= date('now')) ORDER BY season_number`).all(context.series.id) as Array<{ season: number }>).map(row => row.season)
...
async function quickSeriesResults(row: ItemSearchRow, context: { series: any; seasonNumber?: number; episode?: any }, signal: AbortSignal): Promise<any[]> {
  ...
  const target = { tier: context.series.target_tier, resolution: context.series.target_resolution, source: context.series.target_source, codec: context.series.target_codec }
  const results = await searchViaIndexers(indexers, seriesBase(context), {
    categories: [5000], type: 'tvsearch', module: 'series',
    imdbId: context.series.imdb_id, tmdbId: context.series.tmdb_id, tvdbId: context.series.tvdb_id,
  })
...
async function searchSeries(row: ItemSearchRow, signal: AbortSignal): Promise<{ grabbed: boolean; message: string }> {
  ...
  const envelope = {
    ...
    ceiling: { tier: context.series.target_tier, resolution: context.series.target_resolution, source: context.series.target_source, codec: context.series.target_codec },
  }
  const first = await quickSeriesResults(row, context, signal)
  let results = storeResults(row.id, first)
  if (row.mode !== 'quick' && !(row.mode === 'auto' && results.some(result => isWithinQualityEnvelope(parseQualityFromTitle(result.title, scorer), envelope)))) {
    const indexers = getEnabledIndexerInstances()
  ...
  if (context.episode) {
    ...
    getDb().prepare("UPDATE episodes SET status = 'acquiring', info_hash = ?, updated_at = datetime('now') WHERE series_id = ? AND season_number = ? AND status NOT IN ('collected','downloaded')")
    ...
    getDb().prepare("UPDATE episodes SET status = 'acquiring', info_hash = ?, updated_at = datetime('now') WHERE series_id = ? AND status NOT IN ('collected','downloaded')")
      .run(hash, context.series.id)
  }
  return { grabbed: true, message: `Grabbed "${best.title}"` }
}

async function autoEpisodes(row: ItemSearchRow, context: { series: any; seasonNumber?: number }, signal: AbortSignal): Promise<{ grabbed: boolean; message: string }> {
  const target = seriesTarget(context.series)
  const episodes = (getDb().prepare(`SELECT id, episode_number, status, current_tier, current_resolution, current_source, current_codec,
    ...
    .all(context.series.id, context.seasonNumber) as any[]).filter(episode =>
      !['collected', 'downloaded'].includes(episode.status)
      || (hasQualityFloor(target) && !meetsQualityFloor(episodeQuality(episode), target)))
  getDb().prepare("UPDATE item_searches SET message = ?, updated_at = datetime('now') WHERE id = ?")
  ...
  for (const [index, episode] of episodes.entries()) {
    ...
    getDb().prepare("UPDATE episodes SET status = 'acquiring', info_hash = ?, updated_at = datetime('now') WHERE id = ?")
...
async function executeItemSearch(job: JobRecord, signal: AbortSignal): Promise<void> {
  ...
  getDb().prepare(`UPDATE item_searches SET status = 'running', started_at = COALESCE(started_at, datetime('now')),
    error = NULL, updated_at = datetime('now') WHERE id = ?`).run(row.id)
  try {
    const outcome = row.media_type === 'films' ? await searchFilm(row, signal) : await searchSeries(row, signal)
    const expiresAt = new Date(Date.now() + RESULT_RETENTION_MS).toISOString()
    getDb().prepare(`UPDATE item_searches SET status = 'complete', grabbed = ?, message = ?, error = NULL,
    ...
    const expiresAt = new Date(Date.now() + RESULT_RETENTION_MS).toISOString()
    getDb().prepare(`UPDATE item_searches SET status = ?, error = ?, message = ?, completed_at = datetime('now'),
